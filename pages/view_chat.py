...
st.title(f"Chat Between You And {partner_name}")
...
with chat_placeholder.container():    
    for result in results:
        if(result[3] != selected_language):
            tranlated_text = translate_text(
                result[2], src=result[3], dest=language_maplet.language_maplet[selected_language]
            )
            
        if result[1] == user_id:
            message(tranlated_text, is_user=True, key=result[0])
        else:
            message(tranlated_text, key=result[0])
